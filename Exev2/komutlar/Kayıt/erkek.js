...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (!message.guild.roles.cache.get(erkekRol[0]) &&
        ...
        if (kntrl >= 6 && sre > Date.now() && !message.member.permissions.has(8n) && !message.member.roles.cache.some(rol => data.MUTEAuthorized.includes(rol.id))) {
            ...
            await target.roles.remove(unreg).then(async x => {
                ...
                teyit.findOne({guildID: message.guild.id, victimID: target.id}, (err, wex) => {
                    if(!wex) {
                        ...
                        message.reply({ embeds: [Embed.setDescription(`${target} adlı kullanıcı **Erkek** olarak kayıt edildi.`)] }).then(e => setTimeout(() => e.delete().catch(() => { }), 20000)), message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
                    } else {
