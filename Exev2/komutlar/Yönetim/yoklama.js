...
module.exports.run = async (client, message, args, durum, kanal) => {
  ...
  if (message.member.permissions.has(8n) || durum) {
    ...
    let embed = new MessageEmbed()
      .setColor("RANDOM")
      .setTimestamp()
      .setDescription(`Önceki Toplantı: ${data.Katılanlar.length}
Bu Toplantı: ${seste_olanlar.size}

Bu Toplantıya Katılanlar (${seste_olanlar.size})
${seste_olanlar.map(x => `<@${x.id}>`).splice(0, 60)}${seste_olanlar.size > 60 ? "...ve daha fazla" : ""}`)
    ...
    await message.channel.send({embeds: [embed]}).then(async x => {
      ...
      let embed2 = new MessageEmbed()
        ...
        .setTimestamp()
        .setFooter(client.ayarlar.footer)
        .setDescription(`Bu Toplantıya Katılmayanlar: (${seste_olmayanlar.size})
