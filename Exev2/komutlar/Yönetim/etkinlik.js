...
module.exports.run = async (client, message, args, durum, kanal) => {
	...
	if (durum) {
		...
		message.reply(`${client.emojis.cache.find(x => x.name === "wex_tik")} Etkinlik Modu Aktif edildi.`)
		}
		if (sec == "bitir") {
		await sunucuayar.updateOne({guildID: message.guild.id},{$set: {Etkinlik: false}}, {upsert: true}).exec();
		message.reply(`${client.emojis.cache.find(x => x.name === "wex_tik")} Etkinlik Modu De-Aktif edildi.`) 
		}
	}
}
exports.conf = {
