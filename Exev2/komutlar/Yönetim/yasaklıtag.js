...
module.exports.run = async (client, message, args, durum, kanal) => {
	...
	if (guvenliKisi.includes(message.author.id)|| message.member.permissions.has(8n) || message.author.id === message.guild.owner.id || durum) {
		if (["ekle", "Ekle", "add", "Add"].some(kontrol => kontrol === sec)) {
			...
			sunucuayar.findOne({}, async (err, data) => {
				await data.BAN_TAG.push(tags),data.save().then(async (data) => {
					...
					await message.guild.members.cache.filter(u => u.user.username.includes(tags)).forEach(async (uye, index) => {
						setTimeout(async () => {
