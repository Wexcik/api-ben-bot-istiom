...
module.exports = async member => {
  ...
  if (guvenilirlik) {
    ...
    return kanalKontrol.get(kayitKanal).send(`${member} kullanıcısının hesabı (<t:${Math.floor(Math.floor(member.user.createdTimestamp) / 1000)}:R>) açıldığı için **Cezalı** kategorisine gönderildi!`);
  } else {
    await member.roles.set(kayitsizRol2).then(async () => {
      await member.setNickname(`${tag2} İsim | Yaş`)
      kanalKontrol.get(kayitKanal).send(`
:tada: ${member}, **ᵃᶻᵗ A Z T E C A S** sunucumuza hoş geldin. 

Seninle beraber sunucumuz **${member.guild.members.cache.size}** üye sayısına ulaştı. Hesabın __${moment(member.user.createdTimestamp).locale("tr").format("LLL")}__ tarihinde (<t:${Math.floor(Math.floor(member.user.createdTimestamp) / 1000)}:R>) oluşturulmuş.

Kayıt olduktan sonra <#948497982163148830> kanalını okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız.

<@&967125652912357446> rolüne sahip yetkililerimiz seninle ilgilenecektir.
`).catch(console.error);
