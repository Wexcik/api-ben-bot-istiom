...
return;
    ...
    client.channels.cache.get(conf.leaderboard.KanalID).messages.fetch(conf.leaderboard.mesajID).then(m => m.edit({embeds: [teyitEmbed]}))
    // DAVET DATA
    ...
    client.channels.cache.get(conf.leaderboard.KanalID).messages.fetch(conf.leaderboard.mesajID_2).then(m => m.edit({embeds: [davetEmbed]}))
    // TAGLI DATA
    ...
    client.channels.cache.get(conf.leaderboard.KanalID).messages.fetch(conf.leaderboard.mesajID_3).then(m => m.edit({embeds: [tagEmbed]}))
    // TAGLI BİTİŞ
            // YETKİLİ TOP
            ...
            let yetkiliEmbed = new MessageEmbed()
              ...
              client.channels.cache.get(conf.leaderboard.KanalID).messages.fetch(conf.leaderboard.mesajID_4).then(m => m.edit({embeds: [yetkiliEmbed]}))
    ...
    setInterval(async () => {
        ...
        let yetkiliEmbed = new MessageEmbed()
          ...
          .setTitle("Yetkili Alım Tablosu")
          .setDescription(`${yetkiliGoster.length > 0 ? yetkiliGoster : "Veri yoktur"}\n\nBu veriler ${moment(Date.now()).locale("tr").format("LLL")} tarihinde yenilenmiştir.`)
          client.channels.cache.get(conf.leaderboard.KanalID).messages.fetch(conf.leaderboard.mesajID_4).then(m => m.edit({embeds: [yetkiliEmbedh]}))
