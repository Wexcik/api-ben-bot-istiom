  ...
  if (message.member.voice.channel.id === target.voice.channel.id) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")} \`Aynı Kanaldasın.\` Çekmek istediğin kullanıcı ile zaten **Aynı Ses Kanalındasın**.`);
    ...
    .setFooter(conf.footer)
        const row = new MessageActionRow().addComponents(
          ...
          let msg = await message.channel.send({ components: [row], embeds: [embed.setDescription(`Hey ${target}, ${message.author} yanına gelmek istiyor! aşağıda bulunan butonlara tıklayarak odaya gelip gelmemesini belirtebilirsin.`)] }); message.react(`${client.emojis.cache.find(x => x.name === "wex_tik")}`)
          var filter = (button) => button.user.id === target.id;
          const collector = msg.createMessageComponentCollector({ filter, time: 30000 })
          collector.on('collect', async (button, user) => {      
          if(button.customId === "onayla") { msg.delete()
          message.member.voice.setChannel(target.voice.channel.id)
          button.channel.send({embeds: [embed.setDescription(`${client.emojis.cache.find(x => x.name === "wex_tik") || "Emoji Bulunamadı"} ${target} adlı kullanıcı odaya bağlanma istediğinizi onayladı.`)] })}
          if(button.customId === "reddet") { 
          button.channel.send({embeds: [embed.setDescription(`${client.emojis.cache.find(x => x.name === "wex_carpi") || "Emoji Bulunamadı"} ${target} adlı kullanıcı odaya bağlanma istediğinizi reddetti.`)] })
          }
        })
...
exports.conf = {aliases: ["git", "igit"]}
exports.help = {name: 'yanınagit'}
