...
module.exports.run = async (client, message, args, durum) => {
    ...
    let afkEmbed = new MessageEmbed().setDescription("Başarıyla AFK moduna geçtin ve mesajını şu şekilde ayarladım **" + reason + "**.")
    message.reply({embeds: [afkEmbed]}).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
    let newData = afk({
