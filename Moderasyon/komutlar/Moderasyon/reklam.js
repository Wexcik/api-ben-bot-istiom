...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "reklam")) {
        if (jailSorumlusu.length >= 1 && client.channels.cache.get(reklamLOGKanal) && REKLAMLimit >= 1) {
            ...
            if (target.roles.cache.get(reklamROL)) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")} \`Zaten Cezalıda\` Belirttiğin kullanıcı zaten **cezalıda.**`);
            if (limit.get(message.author.id) >= REKLAMLimit) returnmessage.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")} \`Limit Aşımı\` Saatlik **Jail Limitini** doldurduğun için işlem iptal edildi.`);
            ...
            if (target.id === message.author.id) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")} \`Kendine Ceza-i İşlem Uygulayamazsın\` Kendine **Reklam** cezası uygulayamazsın.`);
            ...
            let messageEmbed = `${client.emojis.cache.find(x => x.name === "wex_jailed")} ${target} Adlı üye ${message.author} tarafından **Reklam** gerekçesiyle sunucu üzerinde **Reklamcı** kategorisine gönderildi. (Ceza Numarası: \`#${cezaID+1}\`)`;
