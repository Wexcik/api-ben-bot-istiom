...
module.exports.run = async (client, message, args, durum, kanal) => {
	...
	if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
		...
		if (!target.roles.cache.get(jailRol)) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")} \`Cezalıda Değil?\` Belirttiğin kullanıcı zaten **cezalıda değil**.`);

				await target.roles.set(kayitsizUyeRol)
				message.channel.send(`${client.emojis.cache.find(x => x.name === "wex_tik")} <@${target.id}> adlı üyenin bulunan **Jail** cezasını kaldırdın.`)
