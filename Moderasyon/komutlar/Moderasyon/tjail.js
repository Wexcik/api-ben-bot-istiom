...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
        if (JailSorumlusu.length >= 1 && client.channels.cache.get(JailLogKanal) && JailLimit >= 1) {
            ...
            let messageEmbed = `${client.emojis.cache.find(x => x.name === "wex_jailed")} ${target} Adlı üye ${message.author} tarafından **${reason}** gerekçesiyle **${args[1].replace("h", " saat").replace("m", " dakika").replace("s", " saniye")}** süresi boyunca **jail kategorisine gönderildi.** (Ceza Numarası: \`#${cezaID+1}\`)`
