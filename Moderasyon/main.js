...
moment.locale("tr");
let cezalar = require("./models/ceza")
...
client.on("message", async message => {
  ...
  'Huzur kokuyor geçtiğin her yer.',];
    ...
    module.exports = async (message) => {
        ...
        if ("948822396666781726" && message.channel.id === "948822396666781726" && !message.author.bot) {
            iltifatSayi++;
            if (iltifatSayi >= 100) {
                iltifatSayi = 0;
...
client.on("message", async message => {
  ...
  if (message.member.roles.cache.some(rol => komut.roller.includes(rol.id)) || komut.kisiler.includes(message.author.id) || data2.some(veri => komut.sorumluluk.includes(veri)) || message.member.permissions.has("ADMINISTRATOR")) {
    if (!üye) return message.reply(`${client.emojis.cache.find(x => x.name === "wex_iptal")}\`Üye Belirtilmedi\` Geçerli bir **Üye Belirt** ve tekrar dene.`)
  ...
  if (komut.YetkiliROL === true && üye.user.username.includes(sunucuData.TAG)) {
        client.channels.cache.get(mainSettings.YETKI_VER_LOG).send(new MessageEmbed().setColor("BLACK").setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri alındı! - Yetkiyi Alan: ${message.author} (\`${message.author.id}\`) - ${moment(Date.now()).locale("tr").format("LLL")}`))
...
client.permAyar = async function (userID, guildID, type) {
  ...
  if (type == "unban") {
    durum == member.permissions.has(8n) || member.id == member.guild.owner.id;
  };
  return durum;
};
