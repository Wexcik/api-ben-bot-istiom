...
exports.run = async function (client, message, args, durum, kanal) {
	...
	if (!authorID.some(user => user == message.author.id)) return message.reply("Oyunu sadece oyun başlatıcı bitirebilir!");
		let sestekiUyeler = message.member.voice.channel.members.filter(member => !member.voice.serverDeaf && message.member.id != member.id)
	...
	await vampirKoylu.findOne({Guild: message.guild.id}, (err, veri) => {
		...
		let embedLobi = new MessageEmbed().setColor("RED").setDescription(`Oyunu başlatmak için bir lobi oluşturmalısınız! (\`${prefix}vkstart\`)`)
		if (!data.map(x => x.Channel).some(kanal => kanal === message.member.voice.channel.id)) return message.channel.send({embeds: [embedLobi]})
...
exports.conf = {aliases: ["vkroller"]}
exports.help = {name: 'vkrollercik'}
